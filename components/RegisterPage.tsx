...
const RegisterPage:React.FC<RegisterProp> = (props) => {

    const [isLogin, setIsLogin] = useState(true);
    const [Emessage,setEMsg] = useState<string>(props.error || '');
    ...
    const SignIn = (e?: React.FormEvent) => {
      ...
      .then((response) => {
        if (response?.ok) {
          ...
          setEMsg(response?.error || "Login Failed");
          setOKMsg('');
        }
      })
      .catch((error) => {
        console.error(error);
        setEMsg(error.message || "An error occurred");
        setOKMsg('');
      });
    }

    const [remember,setRem] = useState<boolean>(false);
  
  return (
    <>
        <div className="min-h-screen flex items-center justify-center bg-[#fffdfb]">
              <div className="w-full max-w-md p-8 space-y-6 bg-white overflow-hidden rounded-2xl shadow-xl">
                  ...
                  {!isLogin ? (
                    ...
                    >
                      <div className="space-y-2 text-center">
                        <h1 className="text-3xl text-start font-semibold text-gray-800">Create account</h1>
                        <p className="text-sm text-start text-gray-500">Step {registerStep} of 5</p>
                      </div>
